...
public interface SpringDocAuthController {
    ...
    @ApiErrorResponse(status = HttpStatus.BAD_REQUEST, instance = "/login", errorCases = {
            @ErrorCase(description = "아이디 입력 없음", exampleMessage = "아이디가 입력되지 않았습니다."),
            @ErrorCase(description = "아이디 글자수 오류", exampleMessage = "아이디는 255자 이하로 입력해주세요."),
            @ErrorCase(description = "비밀번호 형식 오류", exampleMessage = "비밀번호는 영어와 숫자를 포함해야합니다."),
    ...
    @ApiErrorResponse(status = HttpStatus.UNAUTHORIZED, instance = "/login", errorCases = {
            @ErrorCase(description = "아이디 불일치", errorCode = ErrorCode.UNAUTHORIZED_ID,
                    exampleMessage = "존재하지 않는 아이디 moly 입니다."),
            @ErrorCase(description = "비밀번호 불일치", errorCode = ErrorCode.UNAUTHORIZED_PASSWORD,
                    exampleMessage = "로그인에 실패하였습니다. 비밀번호를 확인해주세요."),
    })
    ...
    @ApiErrorResponse(status = HttpStatus.UNAUTHORIZED, instance = "/login/check", errorCases = {
            @ErrorCase(description = "쿠키 없음", errorCode = ErrorCode.UNAUTHORIZED_USER,
                    exampleMessage = "쿠키가 없어서 회원 정보를 찾을 수 없습니다. 다시 로그인해주세요."),
            @ErrorCase(description = "인증 쿠키 없음", errorCode = ErrorCode.UNAUTHORIZED_USER,
                    exampleMessage = "인증에 대한 쿠키가 없어서 회원 정보를 찾을 수 없습니다. 다시 로그인해주세요."),
    })
